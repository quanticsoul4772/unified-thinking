...
// toJSONContentWithLevel converts data with a specific format level override
// Use this when you need a specific format level regardless of global setting
func toJSONContentWithLevel(data interface{}, level format.FormatLevel) []mcp.Content {
	if level == format.FormatFull {
		return toJSONContentRaw(data)
	}
...
// toJSONContentRaw converts data to JSON without any formatting
func toJSONContentRaw(data interface{}) []mcp.Content {
	jsonData, err := json.Marshal(data)
	if err != nil {
		errData := map[string]string{"error": err.Error()}
		jsonData, _ = json.Marshal(errData)
